...
describe('countNlpAnnotatingDocuments', () => {
  const documentRepository = buildDocumentRepository();
  const assignationRepository = buildAssignationRepository();
  const userRepository = buildUserRepository();

  it('should return nlpAnnotating documents', async () => {
    ...
    const nlpAnnotatingDocumentsCount = await countNlpAnnotatingDocuments();

    expect(nlpAnnotatingDocumentsCount).toEqual([nlpAnnotatingDocument].length);
  });
});
