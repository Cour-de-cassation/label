  ...
  return (
    <div style={styles.toggle}>
      <div onClick={() => props.onChange('total')} style={buildToggleItemStyle('total')}>
        <Text variant="h3">{wordings.treatmentsPage.table.statistics.computation.total}</Text>
      </div>
      <div onClick={() => props.onChange('average')} style={buildToggleItemStyle('average')}>
        <Text variant="h3">{wordings.treatmentsPage.table.statistics.computation.average}</Text>
      </div>
    </div>
  );
