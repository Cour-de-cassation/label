...
describe('fileNameHandler', () => {
  describe('sortFileNames', () => {
    it('should sort file names', () => {
      ...
      const fileNames = [3, 1, 2, 0].map((order) =>
        fileNameHandler.buildFileName({ id: ids[order], order, extension: 'ts' }),
      );
  ...
  describe('parseFileName', () => {
    it('should extract id and order from file name', () => {
      ...
      const { id, order } = fileNameHandler.parseFileName(fileName);

      expect(idModule.lib.convertToString(id)).toBe('60a28c374703857b2cd08d1b');
  ...
  describe('buildFileName', () => {
    it('should build file name', () => {
      const id = idModule.lib.buildId('60a28c374703857b2cd08d1b');

      const fileName = fileNameHandler.buildFileName({ order: 11, id, extension: 'ts' });
