...
describe('countFreeDocuments', () => {
  const documentRepository = buildDocumentRepository();
  const assignationRepository = buildAssignationRepository();
  const userRepository = buildUserRepository();

  it('should return free documents', async () => {
    ...
    const freeDocumentsCount = await countFreeDocuments();

    expect(freeDocumentsCount).toEqual([freeDocument].length);
  });
});
