...
describe('countSavedDocuments', () => {
  const documentRepository = buildDocumentRepository();
  const assignationRepository = buildAssignationRepository();
  const userRepository = buildUserRepository();

  it('should return saved documents', async () => {
    ...
    const savedDocumentsCount = await countSavedDocuments();

    expect(savedDocumentsCount).toEqual([savedDocument].length);
  });
});
